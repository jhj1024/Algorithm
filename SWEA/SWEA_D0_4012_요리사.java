...
* @caution
* [고려사항] 
* [입력사항] 테스트 케이스의 개수 T, 식재료의 수 N, N * N개의 시너지 정보
* [출력사항] 식재료들을 각각 N / 2개씩 나누어 두 개의 요리를 만들 때, 그 두 요리 맛 차이의 최솟값.
* 
* N개에서 N/2개를 고르는 조합을 만들면 자연스럽게 나머지 N/2개가 만들어짐.
* 두 개의 맛 차이가 적은 것을 고르기
* 
*/

public class SWEA_D0_4012_요리사 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int t = 1; t <= T; t++) {
            ...
            makeCombination(N/2, 0, new int[N/2]); //N개의 요리에서 N/2개를 고르는 조합 생성            
            
            for(int i = 0; i < combi.size()/2; i++) { //i번째 조합의 짝 - combi.size()-i-1번째.
                int cf1 = calFlavor(combi.get(i)); //첫번째 요리의 맛 계산
                int cf2 = calFlavor(combi.get(combi.size()-i-1)); //두번째 요리의 맛 계산
                
                Answer = Math.min(Answer, Math.abs(cf1 - cf2)); //두 음식 맛 차이의 최소값 저장
            }
    ...
    public static int calFlavor(int[] flavors) { //요리 시너지를 모두 더해서 최종 맛 구하기      
        int sum = 0;
        ...
        for(int i = 0; i < N/2; i++) {
            for(int j = 0; j < N/2; j++) {
                if(i == j) {
                    ...
                    sum += map[flavors[i]][flavors[j]]; //요리 시너지 더하기
                }
            }
        }
    ...
    public static void makeCombination(int r, int start, int[] temp) { //N개의 재료에서 N/2개를 고르는 조합
        if(r == 0) { 
            combi.add(Arrays.copyOf(temp, temp.length)); //만들어진 조합은 어레이에 저장
            return;
        }
